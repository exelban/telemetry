...
#########################################################
GPS_port = '/dev/ttyAMA0'  # /dev/ttyAMA0
arduino_port = '/dev/pts/2'  # /dev/ttyUSB1 / /dev/pts/2
#########################################################
    ...
    def receive(self):
        ...
        while True:
            try:
                ...
                print("Please check your GPS module.")
                ...
                try:
                    for c in arduino_serial.read():
                        ...
                        if chr(c) == '\n':
                            ...
                            gps_values = gps_class.gps() if len(GPS_port) is not 0 else ''
                            data = joined_seq
                            date = datetime.datetime.now().strftime("%H:%M:%S:%f")
                            values = re.split(',', data)
                            if len(GPS_port) is not 0:
                    ...
                    print("Pressed Ctrl+C, close app.")
                    if len(GPS_port) is not 0: gps_class.close()
                    break
...
if __name__ == "__main__":
    try:
        _ = serial.Serial(arduino_port, 9600)
        print("Arduino port is ok.")
    except:
        print("Please check arduino port!")
        sys.exit()

    if len(GPS_port) is not 0:
        try:
            _ = serial.Serial(GPS_port, 9600)
            print("GPS port is ok.")
        except:
            print("Please check GPS port!")
            sys.exit()
